...
from app.app import authenticate_user, db
import uuid



@main_bp.route('/property/all_properties')
def get_all_properties():
    page_number = request.args.get('page', 1, type=int)
    # Query the table with all properties
    pagination_properties = Properties.query.filter(
        Properties.active == True).paginate(page=page_number, per_page=20)
...
@main_bp.get('/property/<property_id>')
def get_property(property_id):
    # Query the db for the property of `id`
    result = Properties.query.get(property_id)
...
@main_bp.post('/property/new_property/<realtor_id>')
@authenticate_user
def create_property(realtor_id):
    ...
    new_property = Properties(id=str(uuid.uuid4()),
                            owner_id=realtor_id,
...
# Update property
@main_bp.patch('/property/update_property/<realtor_id>/<property_id>')
@authenticate_user
def update_property(realtor_id, property_id):
    ...
    try:
        ...
        updated_property = Properties.query.get(property_id)
...
@main_bp.patch('/property/update_property_availability/<realtor_id>/<property_id>')
@authenticate_user
...
# Delete property
@main_bp.delete('/property/delete_property/<realtor_id>/<property_id>')
@authenticate_user
def delete_property(realtor_id, property_id):
    ...
    if str(property_details.owner_id) != str(realtor_id):
        return "Not owner", 403
    try:
        db.session.delete(Properties.query.get(property_id))
        db.session.commit()
...
# Search properties
@main_bp.get('/property/search_properties')
def search_properties():
    ...
    query = Properties.query.filter(
        db.or_(
            db.or_(
                func.lower(Properties.location).collate('utf8_general_ci').like(
        func.lower(text('%{}%'.format(search_text)))),
    func.lower(Properties.category).collate('utf8_general_ci').like(
        func.lower(text('%{}%'.format(search_text)))),
    func.lower(Properties.description).collate('utf8_general_ci').like(
        func.lower(text('%{}%'.format(search_text)))),
    func.lower(Properties.address).collate('utf8_general_ci').like(
        func.lower(text('%{}%'.format(search_text))))
            )
        )
    )
    ...
    if len(category):
        query = query.filter(Properties.category == category.lower())
        print("category")
    ...
    if min_price and not (min_price == "" or min_price == "0"):
        query = query.filter(Properties.price >= int(min_price))
        print("min-price")
    ...
    if bathrooms and not (bathrooms == "" or bathrooms == "0"):
        query = query.filter(Properties.bathrooms <= int(bathrooms))
        print("bath")
    if bedrooms and not (bedrooms == "" or bedrooms == "0"):
        query = query.filter(Properties.bedrooms <= int(bedrooms))
        print("beds")
    if max_area and not (max_area == "" or max_area == "0"):
        query = query.filter(Properties.size >= int(max_area))
        print("size")
...
@main_bp.get('/property/recently_added')
def search_recently_added():
    results = Properties.query.order_by(Properties.date_created.desc()).limit(4)
